...
| 4 | `GPU step %q with needs_acceleration should declare fallback_resource_class` | — | — | `test_policy.py::test_deny_gpu_no_fallback` | **Yes** — ground-side reliability; rule requires `resource_class == "gpu"` AND `needs_acceleration == true` AND no `fallback_resource_class` |
