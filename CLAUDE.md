...
### Setup, commands, and cluster bootstrap

Invoke the **dev-environment-setup** skill for: the kubeconfig copy + `.venv-verify`
Python venv + argo/opa CLI install; the canonical pytest / live-cluster-validation
/ ablation / benchmark commands; the one-time Argo v4.0.1 + Kueue v0.17.0 bootstrap;
and the multus `crypto/rsa: verification error` CrashLoop fix.
